...
chk('편집기로 옮길 수 있는 위젯이 스물하나다', _n == 21, '%d개' % _n)   # 💥 한 방 최고 후원이 늘었다   # 🎱 구슬 핀볼이 늘었다   # 🏺 모금함이 늘었다

print()
print('=' * 74)
print('⑩ 편집기 — 안전지대를 걷어냈고 게임 자리는 남아 있는가')
print('=' * 74)
"""🔓 안전지대는 2026-09-15 에 사장님 요청으로 걷어냈다("자꾸 뜨네").

   그전 이력: 게이지가 y 1598, 안내 전광판이 y 1680 — 폰에서 안 보이는 자리로 내려간
   적이 있었고, 그 원인이 편집기에 안전지대 표시가 없어서였다. 그래서 구역 색칠·빨간
   딱지·붙잡기를 넣었었다. 이제 아무것도 안 막는다.

   ⚠️ 위젯이 폰에서 잘리는지는 **사람이 봐야 한다.** 되살릴 일이 생기면 세로
      115～954 가 안 가려지는 구역이다 (0～115 채널줄 · 960～1382 댓글창 · 1382～1920 고정 버튼).
      방송판 기본 자리 검사는 그대로라, HUD 넷이 제자리를 벗어나면 거기서 잡힌다."""
for nm, pat in (('구역 색칠', r'zone-(dead|safe|chat)'),
                ('경계선', r'zone-line'),
                ('빨간 딱지', r'out-of-safe'),
                ("'안전지대 무시' 스위치", r'id="free-on"')):
    chk('편집기에서 %s 을(를) 걷어냈다' % nm, re.search(pat, ad) is None)
