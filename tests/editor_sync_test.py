...
print()
print('=' * 74)
print('💰 금액 딱지가 막대를 따라다니는가')
print('=' * 74)
# ⚠️ 딱지 높이를 화면 고정 구간(115~954)에 가두고 있었다. 그건 기본 자리의
#    막대 구간을 숫자로 적은 것이라 기본 배치에선 같았지만, 편집기에서 게이지를 옷기면
#    막대는 따라가는데 딱지만 그 구간에 붙잡혀 떨어졌다(실측: 상대높이 651 → 521).
#    대표님: "게이지는 잘 움직이는데 옆에 숲자가 안따라간다" (2026-09-16)
chk('딱지를 막대 제 길이에 묶는다',
    'const lo = railTop + half, hi = railTop + railH - half;' in ov)
chk('화면 고정 구간을 안 쓴다', 'const lo = 115 + half' not in ov)
