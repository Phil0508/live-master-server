...
## 🎧 크롬 없이 후원 받기 (toon-listener)

지금까지 후원은 **크롬에 띄운 템퍼몽키가 투네이션 알림창 화면을 긁어** 서버로
보내는 방식이었다. 그래서 방송 내내 크롬을 켜둬야 했다.

`toon_listener.py` 는 그 크롬을 없앤다. 알림창이 내부적으로 쓰는 실시간 소켓
(`wss://ws.toon.at`)에 **서버가 직접 붙어** 후원을 받아 `127.0.0.1:8080/api/donation`
으로 넘긴다. 크롬·템퍼몽키·OBS 브라우저 소스(후원 유입용)가 전부 필요 없어진다.

### 켜는 법

1. 투네이션 알림창 주소를 env 에 넣는다 (통합알림창 > 알림창 URL 의 그 주소):
   ```bash
   sudo nano /etc/livemaster.env
   #   ALERTBOX_URL=https://toon.at/widget/alertbox/<내키>
   ```
2. 켠다:
   ```bash
   sudo systemctl enable --now toon-listener
   sudo journalctl -u toon-listener -f      # 후원 들어오는 게 보인다
   ```
3. 전환: **템퍼몽키 스크립트를 끄고**, OBS 의 후원 알림 브라우저 소스도 필요 없어진다.
   (오버레이·시그니처 표시 소스는 그대로 둔다 — 그건 후원 유입과 무관하다)

### 되는지 미리 보기 (아무것도 안 보냄)

```bash
ALERTBOX_URL=https://toon.at/widget/alertbox/<내키> \
  /opt/livemaster/.venv/bin/python /opt/livemaster/toon_listener.py --dry
```
그리고 투네이션 대시보드에서 **후원 테스트**를 누르면, 서버로 보낼 내용이 화면에만 찍힌다.

### ⚠️ 알아둘 것

- **공식 API 가 아니다.** 투네이션이 알림창 방식을 바꾸면 예고 없이 끊긴다.
  그래서 **기존 템퍼몽키+OBS 방식을 지우지 말고 폴백으로 남겨둔다.** 끊기면 되돌리면 된다.
- 리스너가 죽었는지는 `/health` 페이지의 "마지막 후원 몇 분 전"으로 폰에서 확인된다.
